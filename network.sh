...
DEFAULT_CLI_EXTRA_HOSTS="extra_hosts:[newline]      - orderer.$DOMAIN:$IP_ORDERER[newline]      - www.$DOMAIN:$IP_ORDERER[newline]      - www.$ORG1.$DOMAIN:$IP1[newline]      - www.$ORG2.$DOMAIN:$IP2"
DEFAULT_API_EXTRA_HOSTS="extra_hosts:[newline]      - orderer.$DOMAIN:$IP_ORDERER[newline]      - peer0.$ORG1.$DOMAIN:$IP1[newline]      - peer0.$ORG2.$DOMAIN:$IP2[newline]      - peer1.$ORG1.$DOMAIN:$IP1[newline]      - peer1.$ORG2.$DOMAIN:$IP2"
DEFAULT_API_EXTRA_HOSTS1="extra_hosts:[newline]      - orderer.$DOMAIN:$IP_ORDERER[newline]      - peer0.$ORG2.$DOMAIN:$IP2[newline]      - peer1.$ORG2.$DOMAIN:$IP2"
DEFAULT_API_EXTRA_HOSTS2="extra_hosts:[newline]      - orderer.$DOMAIN:$IP_ORDERER[newline]      - peer0.$ORG1.$DOMAIN:$IP1[newline]      - peer1.$ORG1.$DOMAIN:$IP1"
...
function generateOrdererDockerCompose() {
    ...
    sed -e "s/DOMAIN/$DOMAIN/g" -e "s/MAIN_ORG/$mainOrg/g" -e "s/CLI_EXTRA_HOSTS/$cli_extra_hosts/g" -e "s/ORDERER_PORT/$DEFAULT_ORDERER_PORT/g" -e "s/WWW_PORT/$DEFAULT_WWW_PORT/g" -e "s/ORG1/$ORG1/g" -e "s/ORG2/$ORG2/g" ${compose_template} | awk '{gsub(/\[newline\]/, "\n")}1' > ${f}
...
function generateOrdererArtifacts() {
    ...
    if [[ -n "$org" ]]; then
        ...
        # replace in configtx
        sed -e "s/DOMAIN/$DOMAIN/g" -e "s/ORG1/$ORG1/g" -e "s/ORG2/$ORG2/g" $TEMPLATES_ARTIFACTS_FOLDER/configtxtemplate.yaml > $GENERATED_ARTIFACTS_FOLDER/configtx.yaml
        createChannels=("common" "$ORG1-$ORG2")
    fi
...
function addOrg() {
  ...
  v="2.0"
  policy="OR ('${ORG1}MSP.member','${ORG2}MSP.member','${org}MSP.member')"

  for o in ${ORG1} ${ORG2} ${org}
    do
...
#    createJoinInstantiateWarmUp ${org} common ${CHAINCODE_COMMON_NAME} ${CHAINCODE_COMMON_INIT} ${COLLECTION_CONFIG}
  ...
  dockerContainerRestart $ORG api
elif [ "${MODE}" == "create-channel" ]; then # params: mainOrg($3) channel_name org1 [org2]
  mainOrg=$3
  channel_name=$4
...
elif [ "${MODE}" == "register-org-in-channel" ]; then # params: mainOrg($3) channel_name org1 [org2]
  mainOrg=$3
  channel_name=$4
  echo "Register Orgs in channel $channel_name: ${@:5}"
  for org in "${@:5}"; do
  ...
  [[ -z "$3" ]] && generateNetworkConfig ${ORG1} ${ORG2}
